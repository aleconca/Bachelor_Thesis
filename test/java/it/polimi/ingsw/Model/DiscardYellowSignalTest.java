...
import it.polimi.ingsw.server.Model.DiscardYellowSignal;
import org.junit.Test;

import static org.junit.Assert.*;

public class DiscardYellowSignalTest {

    @Test
    public void testEffect(){
        Game game = new Game();
        remove(game);
        DiscardYellowSignal d = new DiscardYellowSignal();
        d.effect(game);
    ...
    private int numberlvl1(Game game){
        ...
        for (int i=0; i<game.getDevelopCardsDeck().getLvl1().size(); i++){
            if (game.getDevelopCardsDeck().getLvl1().get(i).getColour() == DevelopCard.Colour.Y)
                c++;
        }
        return c;
    }
    ...
    private int numberlvl2(Game game){
        ...
        for (int i=0; i<game.getDevelopCardsDeck().getLvl2().size(); i++){
            if (game.getDevelopCardsDeck().getLvl2().get(i).getColour() == DevelopCard.Colour.Y)
                c++;
        }
        return c;
    }
    ...
    private int numberlvl3(Game game){
        ...
        for (int i=0; i<game.getDevelopCardsDeck().getLvl3().size(); i++){
            if (game.getDevelopCardsDeck().getLvl3().get(i).getColour() == DevelopCard.Colour.Y)
                c++;
        }
        return c;
    }
